...
| ⇄合并 | **0** | 无文件合并 |
| ⇄修复 / ⇄清理（文件级修改） | **49** | 见上表 delta 列 + 附录「49 项修改清单」 |
| =保持 | **81** | 含全部 `dsp/`、`nn/` 数值路径与资产，以及撤下的 D-4 `extractor.py`、C-15 `app.py` |

> **Δ 合计**：新增 1 文件、修改 49 文件、保持 81 文件；**净文件数 130 → 131**（+1）。
> ⚠ 本 delta 为**方案预期值**（派生自三条推导链的机械对账，见附录）——它是「**异常指示器**」，**不是通过标准**。S4 以 `git diff --stat` 实测为准；实际 ≠ 49 时**逐条解释差异**（多改/漏改/降级为零改动）。计数口径：基线 130 = 保持 81 / 修改 49（+1 新增另计）；**D-1 定案**使 `gui/train/view/widgets.py`、`tools_merge.py` 由保持转修改（**+2 改**）；**D-4 撤回**使 `rvc/pipeline/pitch/extractor.py` 由修改转保持（**−1 改**）；**C-15 全量撤回**使 `app.py` 由修改转保持（**−1 改**）→ 汇总 **保持 81 / 修改 49**（见附录 H、附录 F D-3）。
...
```
- 逐条核对 §2.4 delta（**预期**：新增 1 / 修改 49 / 保持 81）——按附录「49 项修改清单」逐条勾对。
...
| 8 | gui/infer/controller/offline_controller.py | A+C | B1/B3 | `OfflineManager`→`OfflineConversion`；删死 `hasattr` |
| 9 | gui/infer/state/bindings.py | A+C | B1/B3/B4 | `params_from_dict(data)`→`state`；删 `hasattr` 探测（P1-4） |
| 10 | gui/infer/view/lifecycle.py | A+C | B1/B3/B4 | `gui_data`→`saved_params`；删死探测（P1-13） |
| 11 | gui/infer/view/tabs/global_params_tab.py | C | B3/B4 | 显式控件属性（P2-10） |
| 12 | gui/infer/view/tray.py | A | B1 | `TrayManager`→`SystemTray`（P1-5；**「经 controller 公开接口触发」子项已撤回**，见附录 H） |
| 13 | gui/infer/view/widgets.py | C | B3/B4 | `_update_label` 提为 `DoubleSlider` 正式方法（P1-5） |
| 14 | gui/infer/view/window.py | A+B+C | B1/B2/B3/B4 | `offline_manager`→`offline_conversion`；删 `_show_info`（B-5）；删死 `hasattr` |
...
| 21 | gui/train/view/window.py | A | B1/B3 | `on_loss(data)`→`loss_report`；`worker.options["epochs"]`→`worker.request.epochs`（B3-3c-b） |
...
> **B4 标记共 12 项**：序号 **8,9,10,11,12,13,14,19,20,21,48,49**（= §⑤ B4 批的文件；其中 `tray.py`(#12) 的「经 controller 公开接口触发」子项已撤回，仅余 `TrayManager`→`SystemTray`）。

**+1 新增**：`gui/infer/view/contracts.py`（★新增，B4）。
**合计 49 修改 + 1 新增 = 50 文件级改动；全量 保持 81 + 修改 49 + 新增 1 = 131**，与 §2.4 定案 delta 自洽。
...
## 附录 D-2 — 已落地批次底账（B1 / B2 / B2.5 / B3-1 / B3-2 / B3-3a / B3-3b / B3-3c）
...
| # | 差异 | 现状 | 影响 | 建议 |
|---|---|---|---|---|
| **D-1**（已消解） | B4（C-6 `ToolRunner`）使 `gui/train/view/widgets.py`、`gui/train/view/tabs/tools_merge.py` 由 `=保持` → **修改** | **已裁定：采纳**（这 2 项计入修改） | 计入后 **保持 79 / 修改 51**；叠加 **D-4 撤回**（`extractor.py` −1 改）→ **保持 80 / 修改 50**；再叠加 **C-15 撤回**（`app.py` −1 改）→ 定案 **保持 81 / 修改 49 / 新增 1** | 已落案（§2.4 / 附录 A / 附录 B） |
...
| C-6 工具线程收敛（＋tray 经 controller 触发） | **B 命中**（tray 子项） | —（窗口级启动 / 停止**调用路径**；贴近 §1–§9「时序 / 存在性」精神 + §7 D-R3 调用顺序冻结） | C-6 本体保留；**tray 触发子项撤回** | §④ C-6 / 附录B #48·#49·#12 |
