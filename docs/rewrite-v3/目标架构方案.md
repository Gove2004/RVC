...
> 全量 **130** 个已跟踪文件（表内逐条列 98；另 32 项为 `__init__`/资产/批处理，统一 `=保持`）。delta 标记：`=保持` / `⇄改名` / `⇄修复` / `⇄清理` / `★新增` / `✗删除`。
...
| `gui/train/view/widgets.py` | 训练控件（含 `ToolThread`） | =保持（B4 或将改判为「修改」：`ToolThread`→`ToolRunner`，见 **待解差异 D-1**） |
...
| `gui/train/view/tabs/tools_merge.py` | 模型合并工具 | =保持（B4 或将改判为「修改」：改用 `win.tool_runner`，见 **待解差异 D-1**） |
...
| =保持 | **81** | 含全部 `dsp/`、`nn/` 数值路径与资产 |

> **Δ 合计**：新增 1 文件、修改 49 文件、保持 81 文件；**净文件数 130 → 131**（+1）。
> ⚠ 本 delta 为**方案预期值**（派生自三条推导链的机械对账，见附录）——它是「**异常指示器**」，**不是通过标准**。S4 以 `git diff --stat` 实测为准；实际 ≠ 49 时**逐条解释差异**（多改/漏改/降级为零改动）。另：B4（C-6 ToolRunner）可能再翻转 `gui/train/view/widgets.py`、`gui/train/view/tabs/tools_merge.py` 两项 → 届时为 79/51，见附录「待解差异 D-1」。
...
| B4 | **View 契约批**（C-14 + C-5/C-6）：★新增 `gui/infer/view/contracts.py`；**改** `gui/infer/view/{lifecycle,window,widgets,tray}.py`、`gui/infer/state/bindings.py`、`gui/infer/view/tabs/global_params_tab.py`、`gui/infer/controller/offline_controller.py`、`gui/train/view/{window,widgets}.py`、`gui/train/view/tabs/{tools_inspect,tools_fixinfo,tools_merge}.py`（共 1 新 + 12 改；其中多数已随 B3 计入，仅 `train/view/widgets.py`、`tools_merge.py` 为此批新增修改，见「待解差异 D-1」） | B3 | `refactor(v3): 显式 View 宿主契约 + 工具线程收敛（行为不变）` |
...
- 详见 `docs/rewrite-v3/S4-数值对照表.md`（A 组 17「必零漂移」 / B 组 12「预期有变更」）。
...
# 附录（阶段二落盘 · 含勘误 / 49 项清单 / B1 底账 / 三链对账）
...
| 10 | gui/infer/state/bindings.py | A+C | B1/B3 | `params_from_dict(data)`→`state`；删 `hasattr` 探测（P1-4） |
...
| 20 | gui/train/view/tabs/tools_fixinfo.py | A | B1 | `_run_fixinfo`→`apply_model_info`、`fix_info`→`model_info`（§12.4） |
...
| 32 | rvc/pipeline/pitch/extractor.py | D | B5 | 报错文案指向真实路径（P1-3） |
| 33 | rvc/pipeline/pitch/postprocess.py | A | B2.5 | 魔法数 `> 128`→具名常量（值不变，P2-6/A-2） |
| 34 | rvc/pipeline/pitch/tracker.py | D | B5 | docstring「最后」→「前 p_len 帧」（代码不动，D-1） |
| 35 | rvc/pipeline/sessions.py | C | B3 | 去 `hasattr(bundle,"synthesizer")` 探测（P1-10）；随 C-3 调整 |
| 36 | rvc/pipeline/synthesis.py | A | B2.5 | 魔法数 `>= 64`@:76 → 具名常量（值不变，A-2）【补漏】 |
| 37 | rvc/runtime/__init__.py | A | B1 | `__all__`/`__getattr__` 同步改名 |
| 38 | rvc/runtime/device.py | A+C | B1/B3 | `Config`→`RuntimeDeviceConfig`；保留「进程内一份」语义（D-7） |
| 39 | rvc/runtime/graph.py | C | B1/B3 | B1 注释同步；B3 `_active_options` 全局→显式下发（P1-6，R-2） |
| 40 | rvc/runtime/paths.py | B | B2 | 删 `FFPROBE_EXE`（文件保留，D-6） |
| 41 | rvc/streaming/effects.py | B | B2 | 删过期 docstring；删 `AudioProcessor.sr`；删 `is_vc` 死分支（P1-15/P2-2） |
| 42 | rvc/streaming/engine.py | A | B1（+B2.5） | `_stream_mgr`→`_streams`；`warmup(30)` 具名常量（B2.5） |
| 43 | rvc/streaming/output.py | C | B3 | `except Exception:pass`→精确捕获 `queue.Empty`（P1-9） |
| 44 | rvc/streaming/runner.py | B | B2 | 删 `state.function=="vc"` 死分支（P1-15/D-4） |
| 45 | rvc/streaming/stream.py | A | B1 | `AudioStreamManager`→`AudioStreams`；局部 `*_record`/`audio_clock` |
| 46 | rvc/train/checkpoint.py | A+D+§12 | B1/B2/B6 | `change_info`→`change_model_info`；安全注释；§12 首行 info |
| 47 | rvc/train/dataset.py | A+C+D | B1/B3 | `data_config`→`mel_config`；6/9 元组→具名 batch（P1-10）；安全注释 |
| 48 | rvc/train/mel_processing.py | B | B2 | 删 `MAX_WAV_VALUE`（P2-2，上轮漏清） |
| 49 | rvc/train/trainer.py | A+C+D | B1/B3 | `data_cfg`→`mel_config`；属性 `__init__` 声明为 None（P1-16）；安全注释 |

**+1 新增**：`gui/infer/view/contracts.py`（★新增，B4）。
**合计 49 修改 + 1 新增 = 131**，与 §2.4 订正 delta 自洽。
...
|---|---|---|---|
| 1 | **§② ⇄集 ↔ 49 项清单** | 双向差集为空 | ✅ 空 |
| 2 | **S4 指引 B 组(12) ⊆ 49 项** | 逐条命中 | ✅ 命中 |
| 3 | **S4 指引 A 组(17) ∩ 49 项 = ∅** | 无交集 | ✅ 空 |

> 三链 = ①§② 分类推导 ②S4 数值冻结路径推导（A 组 17 / B 组 12，见 `docs/rewrite-v3/S4-数值对照表.md`）③类别推导（本 49 项）。**任一处不等即「两张表没对账」的同类问题**，必须在落盘前揪出。本轮据此已揪出并订正：幻影 `rvc/streaming/synthesis.py`、`autodl/steps.py` 漏列、`rvc/models/__init__.py` 误列、`rvc/nn/modules.py` 误列、三个高危点位定位。

## 附录 F — 待解差异（不掩盖 · 报 team-lead）

| # | 差异 | 现状 | 影响 | 建议 |
|---|---|---|---|---|
| **D-1** | B4（C-6 `ToolRunner`）会使 `gui/train/view/widgets.py`、`gui/train/view/tabs/tools_merge.py` 由 `=保持` → **修改** | 本表按 team-lead 口令记 **81/49**，未计入这 2 项 | 预期或应为 **79/51** | 待 team-lead 裁定是否计入；S4 逐条解释覆盖 |
| **D-2** | B1 实测**原漏列 `autodl/steps.py`**（`on_loss` 改名），原表却含 `rvc/models/__init__.py`（实测无改名） | 二者相抵，**49 总数不变**；已按实测订正组成（附录 A） | 无（已订正） | 已订正，记录在案 |
