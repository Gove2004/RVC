...
│  │   ├─ synthesis.py             ⇄改名  魔法数 → 具名常量（A-2，B2.5 已改）；docstring 对齐调用路径（B5 已改）
...
| 2026-09-23 | **§② 树状图陈旧登记** | `rvc/pipeline/synthesis.py` 标 `=保持`（§② :73） | 与附录 B #33 / §②映射表（A-2 魔法数 `>= 64`→具名常量，**B2.5 已落盘**）矛盾 → 订正为 `⇄改名`；**delta 计数不变**（该文件本就在修改 50 内），B5 的 docstring 对齐仅在其修改要点上追加，不产生新的文件级改动 |
| 2026-09-23 | **B6 越权自纠** | §12.4 消歧时顺手改了 `tools_tab.py:1` docstring「修正真名」 | 该文件登记 `=保持`，且 §12.4 具名范围仅 `tools_inspect`/`tools_fixinfo` 两文件 → **已回退原文**，维持 =保持；**delta 计数不变 80/50/1**（`train_controller.py` 的 docstring 消歧保留：该文件本就在修改 50，且其 docstring 描述的正是 §12.4 改名的 API） |
...
| # | 文件 | 类别 | 批次 | 变更要点 |
|--:|---|---|---|---|
| 1 | README.md | D | B5 | 结构/能力如实化（删 tests/92 用例/守护描述；`loudness.py`→`effects.py`、`data_utils.py`→`dataset.py`，D-2；**B5 已改**） |
...
| 15 | gui/train/controller/train_controller.py | A+C | B1/B3/B6 | `change_info`→`change_model_info`（§12.4）；`validate_options` 返回 `TrainRequest`（B3-3c-b）；docstring「真名」消歧（§12.4，**B6 已改**） |
...
| 19 | gui/train/view/tabs/tools_fixinfo.py | A+D | B1/B4/B6 | `_run_fixinfo`→`apply_model_info`、`fix_info`→`model_info`（§12.4）；UI 文案消歧（标题/label/placeholder/警告「真名」→「模型信息」，§12.4/D-6，**B6 已改**） |
| 20 | gui/train/view/tabs/tools_inspect.py | §12 | B1/B4/B6 | §12.3 解析目标「首行」→「zip 原名行」（**B6 已改**；解析不到该行时改名框留空，不填 info/stem） |
...
| 30 | rvc/pipeline/pitch/tracker.py | D | B5 | docstring「最后」→「前 p_len 帧」（代码不动，D-1；**B5 已改**） |
...
| 33 | rvc/pipeline/synthesis.py | A+D | B2.5/B5 | 魔法数 `>= 64` → 具名常量（值不变，A-2，B2.5 已改）；docstring 对齐调用路径现实（唯一调用点恒传 skip_head，offline 分支不可达、按现状保留；**B5 已改**） |
...
| 39 | rvc/streaming/engine.py | A | B1（+B2.5/B5） | `_stream_mgr`→`_streams`；`warmup(30)` 具名常量（B2.5）；§11.2 无条件清 F0 图提为显式命名步骤 `_clear_f0_cuda_graph_caches`（**仅命名与注释，时机/取值不变；B5 已改**） |
...
| 43 | rvc/train/checkpoint.py | A+D+§12 | B1/B2/B6 | `change_info`→`change_model_info`；安全注释；§12 首行 info + zip 原名独立行（**B6 已改**：L1 `模型信息:` 优先 info、空则回退 zip 原名/stem；L2 `zip 原名:` 无条件输出——archive=None 时如实显示 None，据此改名被 `change_archive_name` 响亮拒绝；L3–L6 不变） |
