...
│  ├─ models/          [L4a]
│  │   ├─ hubert.py                ⇄修复  手工换流 → 统一 _suppress_third_party_output contextmanager（条件保留 + 三条等价要求，见附录 H）
...
| A-2 | 魔法数具名化：`rvc/pipeline/synthesis.py:76`（`>= 64` 整表清空，`apply_formant_resample`）、`rvc/pipeline/pitch/postprocess.py:79,97`（`> 128` 淘汰）、`rvc/core/config.py:38`（40ms 上限）、`rvc/streaming/engine.py:159`（预热 30 次）、**`autodl/__init__.py`（`WIZARD_*`：epoch=250 / save_every=20 / per=3.7 → 基线断言 §11.4 / §11.5 / §8.19，B3-1 已改）** → 大写常量，**字面值不变** | **零** |
...
| C-11 | `hubert.load_hubert` 手工换流 → 统一 `_suppress_third_party_output` 同款 **contextmanager**（P1-8）【**条件保留**：已核 §1–§9 无断言命中；保留须满足等价要求 ①`stdout`+`stderr` 皆重定向 ②`devnull` 构造/关闭时机不变 ③恢复顺序与异常路径不变】 | **零**（须满足上述三条等价要求） |
...
| B3 | **隐式契约显式化批**（类别 C：C-1…C-14，**除** C-5/C-6/C-14 归 B4；**C-15 已全量撤回**）——实际拆 **B3-1**（启动期与探测契约）· **B3-2**（CUDA Graph 显式持有） | B1 | `refactor(v3): 显式化全局/探测/位置解包契约（行为不变）` |
...
| 6 | gui/infer/controller/device_controller.py | C | B3 | 复用 `rescan_audio_devices`（P1-11 / §11.10） |
...
| 15 | gui/train/controller/train_controller.py | A | B1 | `change_info`→`change_model_info`（§12.4） |
| 16 | gui/train/controller/workers.py | A+C | B1/B3 | `Config`→`RuntimeDeviceConfig`；`options` dict→`TrainRequest`（P1-10） |
| 17 | gui/train/state/train_state.py | A+C | B1/B3 | 内部键单一来源（★三对外默认 20/200 各自不变★，D-9） |
...
| 21 | gui/train/view/window.py | A | B1 | `on_loss(data)`→`loss_report` |
| 22 | rvc/core/errors.py | B | B2 | 删 `ConfigError`（无人抛/捕） |
| 23 | rvc/io/devices.py | A+C | B3 | `_rescan_portaudio_devices`→公开 `rescan_audio_devices`（§11.10） |
| 24 | rvc/io/wav_file.py | A | B1 | `data*`→`samples`/`pcm_*`；`read_*_info`→`read_*_metadata` |
| 25 | rvc/models/hubert.py | C | B3 | 手工换流→`_suppress_third_party_output` 同款 contextmanager（P1-8；**条件保留** + 三条等价要求，见附录 H） |
| 26 | rvc/models/rmvpe/model.py | D | B2 | `weights_only=False` 补安全注释（P1-12） |
| 27 | rvc/pipeline/cache.py | C | B3 | `default_inference_cache` 全局→实例归属（P1-6） |
| 28 | rvc/pipeline/context.py | C | B3 | `reset()` 支持 `default_factory`（P1-14） |
| 29 | rvc/pipeline/loader.py | D | B1/B2 | B1 已改同步引用；B2 补 `weights_only` 安全注释（P1-12） |
| 30 | rvc/pipeline/pipeline.py | C | B3 | C-3 波及 `default_inference_cache`（:23/:59）【补漏】 |
| 31 | rvc/pipeline/pitch/postprocess.py | A | B2.5 | 魔法数 `> 128`→具名常量（值不变，P2-6/A-2） |
| 32 | rvc/pipeline/pitch/tracker.py | D | B5 | docstring「最后」→「前 p_len 帧」（代码不动，D-1） |
| 33 | rvc/pipeline/sessions.py | C | B3 | 去 `hasattr(bundle,"synthesizer")` 探测（P1-10）；随 C-3 调整 |
| 34 | rvc/pipeline/synthesis.py | A | B2.5 | 魔法数 `>= 64`@:76 → 具名常量（值不变，A-2）【补漏】 |
| 35 | rvc/runtime/__init__.py | A | B1 | `__all__`/`__getattr__` 同步改名 |
| 36 | rvc/runtime/device.py | A+C | B1/B3 | `Config`→`RuntimeDeviceConfig`；保留「进程内一份」语义（D-7） |
| 37 | rvc/runtime/graph.py | C | B1/B3-2 | B1 注释同步；B3-2 `_active_options` 全局→显式持有（P1-6，R-2） |
| 38 | rvc/runtime/paths.py | B | B2 | 删 `FFPROBE_EXE`（文件保留，D-6） |
| 39 | rvc/streaming/effects.py | B | B2 | 删过期 docstring；删 `AudioProcessor.sr`；删 `is_vc` 死分支（P1-15/P2-2） |
| 40 | rvc/streaming/engine.py | A | B1（+B2.5） | `_stream_mgr`→`_streams`；`warmup(30)` 具名常量（B2.5） |
| 41 | rvc/streaming/output.py | C | B3 | `except Exception:pass`→精确捕获 `queue.Empty`（P1-9；**仅 narrowing，已裁定**，见附录 H） |
| 42 | rvc/streaming/runner.py | B | B2 | 删 `state.function=="vc"` 死分支（P1-15/D-4） |
| 43 | rvc/streaming/stream.py | A | B1 | `AudioStreamManager`→`AudioStreams`；局部 `*_record`/`audio_clock` |
| 44 | rvc/train/checkpoint.py | A+D+§12 | B1/B2/B6 | `change_info`→`change_model_info`；安全注释；§12 首行 info |
| 45 | rvc/train/dataset.py | A+C+D | B1/B3 | `data_config`→`mel_config`；6/9 元组→具名 batch（P1-10）；安全注释 |
| 46 | rvc/train/mel_processing.py | B | B2 | 删 `MAX_WAV_VALUE`（P2-2，上轮漏清） |
| 47 | rvc/train/trainer.py | A+C+D | B1/B3 | `data_cfg`→`mel_config`；属性 `__init__` 声明为 None（P1-16）；安全注释 |
...
> **撤回 2 项**：①原表 `rvc/pipeline/pitch/extractor.py`（报错文案指向真实路径）系**越权**（对外可见字符串，不在 §12）→ 已撤下，改判 `=保持`；②原表 `app.py`（C-15：import 期副作用收敛）系**越权**（变更 import 时机：《功能基线清单》§1.4–1.7 / §1.18 断言其**时机本身**，§1.6 明示「挪进 `main()` 即违约」）→ 已撤下，改判 `=保持`（见附录 H）。
...
## 附录 D-2 — 已落地批次底账（B1 / B2 / B3-1 / B3-2）
...
| 10 | `6046638` | 文档（**本批**：C-15 撤回 + C 类自查 + 底账） | 2 | 162 / 96 | 无 |
...
> **注 2（B2 实测文件 ⊃ 附录B 登记）**：B2 实际 17 文件，其中 `rvc/pipeline/state.py`、`rvc/train/preprocess.py` **未在 §②/附录B 登记** → 见附录 F **D-3**（待 team-lead 复核；涉及定案 81/49 vs 79/51）。
> **注 3（快照口径）**：`bc5ecb6`(B3-2)、`eabb098`(B2.5)、`0bc3810`(B3-3a) 于本口令下达后由工程师陆续落地，均已并入——**本底账为 `6046638` 时点快照**，此后新批由工程师持续追加。上述新批**所触文件全部已在 §②/附录B 登记**（不含 D-3 两文件，故不影响 D-3 结论）。
...
| **D-3**（**待 team-lead 复核**） | **B2 补登记实测 17 文件**，其中 2 个**不在 §②/附录B**：`rvc/pipeline/state.py`（删死字段 `function: str = "vc"`，§④ B-7 只写了 `runner.py`/`engine.py`）、`rvc/train/preprocess.py`（删死函数 `manifest_matches`，§④ B-3 未列文件） | 二者均属 §④ B 类**已叙机制**，仅 §②/附录B **漏列文件**（git 可验：`git show 4b79918 -- rvc/pipeline/state.py` / `-- rvc/train/preprocess.py`） | 若纳入：base 应 = 保持 **79** / 修改 **51**，定案应 = **保持 79 / 修改 51 / 新增 1**（而非 81/49） | 已上报 team-lead；**未获裁定前维持 81/49**；裁定后 5 分钟内：§② 两行改 ⇄清理 + 附录B +2 行 + 三链重核 |
...
|---|---|---|---|---|
| A-1 内部符号改名 | 均否 | —（对外串 `save_state.json` 键 / CLI 参数 / 报错语义一律不动，§3 反向红线） | 保留 | §3.2 / 附录B #4–#47 |
| A-2 魔法数具名化 | 均否 | —（字面值不变） | 保留 | §④ A-2 / 附录B #31·#34·#40 |
| A-3 局部变量语义化 | 均否 | — | 保留 | §④ A-3 |
| B-1 删 `MAX_WAV_VALUE` | 均否 | —（零引用死常量） | 保留 | 附录B #46 |
| B-2 删 `FFPROBE_EXE` | 均否 | —（死常量；文件保留 D-6） | 保留 | 附录B #38 |
| B-3 删 `spectral_de_normalize_torch` / `manifest_matches` | 均否 | —（零引用死函数） | 保留（**⚠ 漏列 `rvc/train/preprocess.py`，见附录F D-3**） | 附录B #46 |
| B-4 删 `ConfigError` | 均否 | —（无人抛/捕） | 保留 | 附录B #22 |
| B-5 删 `_show_info` / `_state_defaults` | 均否 | —（死方法 / 死赋值） | 保留 | 附录B #14·#18 |
| B-6 删 `AudioProcessor.sr` | 均否 | —（赋值后无读取） | 保留 | 附录B #39 |
| B-7 删 `state.function` 恒 `"vc"` 分支 | 均否 | —（不可达分支） | 保留（**⚠ 漏列 `rvc/pipeline/state.py`，见附录F D-3**） | 附录B #42 |
| B-8 删 `_load_thread` 空转 | 均否 | —（恒 `None`） | 保留 | 附录B #7 |
| B-9 删过期 docstring（＋原含 extractor 报错文案） | **A 命中**（报错文案） | —（非 §1–§9；属 §12 边界） | docstring 保留；**extractor 子项撤回** | 附录B #39 / 附录H D-4 |
| C-1 `Config` 单例→显式注入 | 均否 | —（构造次数不变） | 保留 | §④ C-1 / 附录B #36 |
| C-2 `_active_options`→显式持有 | 均否 | —（写 / 读点与时机不变，D-R4） | 保留 | §④ C-2 / 附录B #37 |
| C-3 `default_inference_cache` 全局→实例 | 均否 | —（同进程共享一份的效果保留） | 保留 | §④ C-3 / 附录B #27·#30 |
| C-4 `sessions` 去 `hasattr` 探测 | 均否 | —（探测恒真） | 保留 | §④ C-4 / 附录B #33 |
...
| C-8 元组→dataclass / `options`→`TrainRequest` | 均否 | —（字段值与顺序一致） | 保留 | §④ C-8 / 附录B #16·#45 |
| C-9 静默吞异常 narrowing（＋新增可见消息） | **A 命中**（新增 `logger.warning` / 文案） | —（新可见串不在 §12） | 仅保留 **narrowing**；**新增可见消息子项撤回** | §④ C-9 / 附录B #41 / 附录H |
| C-10 复用 `rescan_audio_devices` | 均否 | —（同一次 terminate/init，§11.10） | 保留 | §④ C-10 / 附录B #6·#23 |
| C-11 `hubert` 换流→contextmanager | 均否 | **已核 §1–§9 无命中** | **条件保留**（三条等价要求） | §④ C-11 / 附录B #25 |
| C-12 `reset()` 支持 `default_factory` | 均否 | —（当前全为字面默认） | 保留 | §④ C-12 / 附录B #28 |
| C-13 `Trainer` 属性 `__init__` 声明 None | 均否 | — | 保留 | §④ C-13 / 附录B #47 |
...
| D-1 `tracker` docstring | 均否 | —（注释非运行时可见） | 保留 | 附录B #32 |
| D-2 README 如实化 | 均否 | —（文档） | 保留 | 附录B #1 |
| D-3 `effects` docstring 对齐 | 均否 | — | 保留 | 附录B #39 |
| D-4 `extractor` 报错文案 | **A 命中** | —（运行时可见；不在 §12） | **撤回** | 附录H / §② |
| D-5 9 处 `weights_only` 安全注释 | 均否 | —（仅注释） | 保留 | 附录B #26·#29·#44·#45·#47 |
| D-6「真名」消歧文案 | **A 命中 · 但已授权** | — | 保留（§12.4 授权） | 附录B #19·#20 |

> **本轮撤回清单（4 个子条目）**：**D-4**（extractor 报错文案）、**C-15 全量**（`app.py` + `autodl` 子项）、**C-9 的「新增可见消息」子项**、**C-6 的 tray 触发子项**。**A / B 两类其余条目、C 类其余 14 条、D 类其余 5 条**，经逐条比对 §12 与 §1–§9，**均无越权**。
