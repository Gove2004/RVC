...
def build_global_params_tab(win):
    ...
    # ── 采样与融合参数 ──
    win.block_time_slider = _slrow(win, "block_time_slider", 0.05, 0.50, 0.01, win.runtime_params.block_time if hasattr(win, "runtime_params") else 0.25)
    g.addWidget(QLabel("采样长度"), r, 0); g.addWidget(win.block_time_slider, r, 1); g.addWidget(win.block_time_label, r, 2); r += 1

    win.crossfade_slider = _slrow(win, "crossfade_slider", 0.01, 0.05, 0.01, win.runtime_params.crossfade_time if hasattr(win, "runtime_params") else 0.05)
    g.addWidget(QLabel("淡入长度"), r, 0); g.addWidget(win.crossfade_slider, r, 1); g.addWidget(win.crossfade_label, r, 2); r += 1

    win.extra_time_slider = _slrow(win, "extra_time_slider", 0.10, 5.0, 0.10, win.runtime_params.extra_time if hasattr(win, "runtime_params") else 2.5)
    g.addWidget(QLabel("额外上下文"), r, 0); g.addWidget(win.extra_time_slider, r, 1); g.addWidget(win.extra_time_label, r, 2); r += 1

    win.rms_mix_slider = _slrow(win, "rms_mix_slider", 0.0, 1.0, 0.05, win.runtime_params.rms_mix if hasattr(win, "runtime_params") else 0.0)
    ...
    win.formant_slider = _slrow(win, "formant_slider", -2.5, 2.5, 0.05, win.runtime_params.formant if hasattr(win, "runtime_params") else 0.0, fmt="+.2f")
