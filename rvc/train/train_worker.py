...
from rvc.train.preprocess import PreProcessor, generate_filelist
...
class TrainWorker(QThread):
    ...
    def _step_f0(self, config, exp_dir, sr):
        ...
        F0Extractor(config.device, config.is_half).run(str(exp_dir), self.progress.emit)
    ...
    def _step_feature(self, config, exp_dir, sr):
        ...
        HuBERTExtractor(config.device, config.is_half).run(str(exp_dir), self.progress.emit)
