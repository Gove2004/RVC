...
def upsample_features(
    ...
    protect: float = 0.5,
    protect_threshold_hz: float = 30.0,
) -> torch.Tensor:
    """特征上采样：50fps → 100fps（线性插值），截取 p_len 帧。

    清辅音保护：对 F0 < protect_threshold_hz 的清音帧混合原始输入特征（feats0），
    ...
    Args:
        ...
        pitchf: 连续 F0 (B, T)，F0 < threshold 的帧视为清音；None 时不保护
        protect: 保护强度（0-1，0=不保护，1=完全保护）
        protect_threshold_hz: 清浊判定阈值（Hz），F0 低于此值视为清音
    ...
    if feats0 is not None and pitchf is not None and protect > 0:
        ...
        uv_mask = (pitchf_aligned < protect_threshold_hz).float()[:, :, None]  # [B, T, 1]
