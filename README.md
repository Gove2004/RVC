...
- **实时变声** — 麦克风输入实时转换输出，延迟实时实测显示（硬件时间戳）
...
rvc/
  audio/
    realtime_engine.py      # RealtimeEngine（音频流管理 + 回调编排）
...
- **延迟实测** — 基于 PortAudio 硬件时间戳（ADC 采集 → DAC 播放）实时测量端到端延迟，替代不可靠的估算值
...
- **延迟显示**：运行中显示的是硬件时间戳实测值（`outputBufferDacTime - inputBufferAdcTime`，含设备缓冲/攒块/推理全链路）；首次冷启动可能偏高（音频栈/GPU 预热），停止再开始即恢复正常
...
**Q: 延迟显示的数值准确吗？**  
A: 运行中显示的是 PortAudio 硬件时间戳实测的端到端延迟（声卡时钟域，含设备缓冲/攒块/推理），比估算更贴近真实听感。首次冷启动偏高属正常（音频栈/GPU 预热），停止再开始即恢复正常。
