...
- **音高调节** — 变调不变速（-16 ~ +16 半音）
- **辅音保护** — 保留原音清音/辅音，防止齿音失真（0.0 ~ 1.0，支持软阈值模式）
- **破音保护** — 高音超过临界 Hz 自动软收敛，防止破音/沙哑（开关 + 临界 Hz，压缩比/膝宽内部自动）
- **双输出** — 主输出 + 可选副输出（虚拟音频设备）
- **实验性功能** — 辅音保护软阈值等实验性功能开关（`rvc/core/experimental.py`）
...
gui/
  ...
  infer/                    # 推理 GUI（MVC 分层）
    view/                   # View 层（纯 UI 构建 + 信号连接）
      ...
      tabs/                 # 各功能 Tab
        audio_driver_tab.py # 设备 Tab
        experimental_tab.py # 实验功能 Tab
        global_params_tab.py# 参数 Tab
        models_tab.py       # 模型 Tab
...
tests/                      # 单元测试（unittest，无需额外依赖，1152 个测试）
...
- **辅音保护** — 使用 F0 contour 作为掩码，清音区域混回原始特征；支持硬阈值和软阈值（sigmoid）两种模式
- **破音保护** — 压缩比 + 平滑膝三段映射，高音区不再压死到固定顶，保留相对旋律
...
**Q: 辅音保护（protect）如何调节？**
A: 0.0 = 完全转换（音色纯但可能糊），1.0 = 完全保留原音辅音（清晰但音色不纯）。建议从 0.5 开始调整。实验性功能中可开启软阈值模式，减少浊音/清音边界突变。
