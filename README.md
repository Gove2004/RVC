...
- **音频效果** — 3 段 EQ（FFT 频域）+ 混响 + 谱减法降噪 + RMS 响度匹配
...
5. **开始推理**
   ...
   - 点击"开始"启动实时推理
6. **音频效果**（可选）
   - 在"效果" Tab 分别启用空间混响、谱减法降噪、音效处理（3 段 EQ）：
     - **空间混响**：0.00 ~ 1.00
     - **谱减法降噪**：输入侧降噪（GPU 谱减法、零新增延迟），强度 0.00 ~ 1.00
     - **3 段 EQ**：低频(200Hz) / 中频(1kHz) / 高频(8kHz)
     - **预设**：原声 / 萝莉 / 少女 / 少御 / 御姐 / 辣条
7. **背景音**（可选）
   - 在"背景" Tab 勾选"开启背景音"、选择本地音频文件、调节音量（0.00 ~ 1.00）
   - 背景音在推理启动时载入并自动循环播放，叠加到最终输出
   - 开关与音量在推理过程中实时生效，无需停止/重启
   - 支持 wav / mp3 / flac / m4a / ogg / aac 等格式（通过 ffmpeg 解码）
